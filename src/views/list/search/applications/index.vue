				...
				<vui-list v-else v-bind:grid="list.grid" v-bind:data="list.data">
					<vui-list-item slot="item" slot-scope="item, index">
						<vui-card v-bind:bordered="false" shadow="always">
							...
							<div class="vui-pro-list-search-applications-item-statistic">
							...
							<template slot="actions">
								...
								<vui-dropdown style="display: block;" placement="bottom">
									...
									<vui-dropdown-menu slot="menu">
										...
										<vui-dropdown-menu-item name="2" title="下载" />
										<vui-dropdown-menu-item name="3" title="分享" />
										<vui-dropdown-menu-item name="4" title="删除" />
									</vui-dropdown-menu>
								</vui-dropdown>
							</template>
						</vui-card>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				this.$store.dispatch("example/getListSearchApplicationsDatasource", payload).then(data => {
...
<style>
	.vui-pro-list-search-applications-item-statistic { display:flex; align-items:flex-start; margin-top:8px; padding-left:48px; }
	.vui-pro-list-search-applications-item-statistic .vui-statistic { width:50%; }
</style>